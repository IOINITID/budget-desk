...
import { fmtMoney, fmtMoneyCompact, RU_MONTH_SHORT, periodLengthDays, dayIndexInPeriod, fmtISO, fmtDate, parseISO, periodLabel } from './data';

/* ---------- Sidebar ---------- */
export function Sidebar({ current, onNav, lastExport }) {
  const items = [
...
export function TopBar({ title, period, onPrev, onNext, onToday, rightSlot }) {
  return (
...
/* ---------- Stat card ---------- */
export function StatCard({ title, value, sub, accent, hint, currency='₽' }) {
  return (
...
/* ---------- Bar row ---------- */
export function BarRow({ color, label, value, plan, currency = '₽', clickable, onClick }) {
...
/* ---------- Donut (svg) ---------- */
export function Donut({ data, size = 130, thickness = 18 }) {
...
export function LineChart({ data, height = 110, color = 'var(--accent)', secondary, secondaryColor = 'var(--text-dim)', showAxis = true, labels }) {
...
export function PlanFactBars({ rows, currency = '₽' }) {
...
export function SpendHeatmap({ period, dailyTotals, today }) {
...
export function CatChip({ category, useEmoji }) {
...
export function IconTile({ category, account, size = 26, useEmoji }) {
...
/* ---------- Banner ---------- */
export function Banner({ icon = '!', children, onDismiss, actions }) {
  return (
...
export function DailyBars({ dailyTotals, period, today, targetPerDay, forecastFrom, forecast, height = 130 }) {
